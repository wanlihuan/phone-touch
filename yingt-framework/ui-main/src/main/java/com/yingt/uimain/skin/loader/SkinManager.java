...
import com.yingt.uimain.skin.listener.ILoaderListener;
import com.yingt.uimain.skin.listener.ISkinLoader;
...
public class SkinManager implements ISkinLoader {
            ...
            protected Resources doInBackground(String... params) {
                try {
                    if (params.length == 1) {
                        ...
                        // 完成皮肤从 assets 复制到 sd 卡
                        String assetsSkin = "";
                        if(!TextUtils.isEmpty(skinPkgPath)) {
                            String[] assetsSkinPaths = skinPkgPath.split("kds/");
                            if(assetsSkinPaths != null && assetsSkinPaths.length > 1)
                                assetsSkin = assetsSkinPaths[1];
                        }
                        FileSystem.copyFileFromAssets(context, assetsSkin, skinPkgPath);
    ...
    /**
     * 获取assets 目录下的包路径文件
     * @param assetSkinPackagePath assets 目录下的包路径文件
     * @return
     */
    public String getAssetsSkinPackagePath(String assetSkinPackagePath){
        File skin = FileSystem.getSDCardCacheFile(context, assetSkinPackagePath);// assets 的目录下路径文件
