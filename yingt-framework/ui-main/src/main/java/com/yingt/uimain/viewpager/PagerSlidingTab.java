...
public class PagerSlidingTab extends HorizontalScrollView {
    ...
    private boolean mLastItemHasIcon = false;
    ...
    public void notifyDataSetChanged() {
        ...
        for (int i = 0; i < mTabCount; i++) {
            if (isCustomTabs) {
                ...
                tabView = LayoutInflater.from(getContext()).inflate(R.layout.yingt_uimain_psts_tab, this, false);
            }
...
//        final FrameLayout frameLayout = (FrameLayout) tabView.findViewById(R.id.yingt_uimain_psts_tab_right);
        ...
        tabView.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                if (mPager.getCurrentItem() != position) {
                    ...
                    unSelect(tab);
//                    if(Math.abs(mPager.getCurrentItem() - position) > 1)
//                    	mPager.setCurrentItem(position, false);//保持切换时page不会滑动，以免太多页时要滑很久才能滑到目标项
//                    else
                    	mPager.setCurrentItem(position);

                    // add by laihuan.wan on 2017/06/30
                    // 处理最后一项的指示器 icon
                    if(mLastItemHasIcon && mTabsContainer.getChildCount() > 0) {
                        FrameLayout frameLayout = (FrameLayout) mTabsContainer.getChildAt(mTabsContainer.getChildCount() - 1)
                                .findViewById(R.id.yingt_uimain_psts_tab_right);
                        View view = null;
                        if (frameLayout.getChildCount() > 0)
                            view = frameLayout.getChildAt(0);
                        if (view != null && mTabsContainer.getChildCount() - 1 == mPager.getCurrentItem() && !view.isSelected()) {
                            view.setSelected(true);// 向上
                            ObjectAnimator objectAnimator = ObjectAnimator.ofFloat(view, View.ROTATION, 0, 180);
                            objectAnimator.setDuration(300);
                            objectAnimator.start();
                        } else if (view != null && view.isSelected()) {
                            view.setSelected(false);// 向下
    ...
    private void updateTabStyles() {
        for (int i = 0; i < mTabCount; i++) {
            ...
            TextView tab_title = (TextView) v.findViewById(R.id.yingt_uimain_psts_tab_title);

            if(mLastItemHasIcon && i == mTabCount-1) {
                FrameLayout frameLayout = (FrameLayout) v.findViewById(R.id.yingt_uimain_psts_tab_right);
                ImageView imageView = new ImageView(getContext());
                imageView.setImageDrawable(getContext().getDrawable(R.drawable.yingt_uimain_pst_indica_icon));
...
//            mRectPaint.setColor(Color.parseColor("#99FF0000"));
            Pair<Float, Float> lines = getIndicatorCoordinates();
//            RectF Oval = new RectF(lines.first + mPaddingLeft, 0, lines.second + mPaddingLeft, height - mIndicatorHeight);
//            canvas.drawOval(Oval, mRectPaint);
//            canvas.drawCircle(lines.first + mPaddingLeft + (lines.second - lines.first)/2, height / 2, (height - mIndicatorHeight) / 2, mRectPaint);
            canvas.drawRect(lines.first + mPaddingLeft + mIndicatorLinePadding, height - mIndicatorHeight,
                    lines.second + mPaddingLeft - mIndicatorLinePadding, height, mRectPaint);
        }
    }
    ...
    private void unSelect(View tab) {
        if (tab != null) {
            TextView tab_title = (TextView) tab.findViewById(R.id.yingt_uimain_psts_tab_title);
            if (tab_title != null) {
    ...
    public void setTextColor(int normalClor, int selectedColor){
